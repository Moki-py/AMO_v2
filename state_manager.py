...
class StateManager:
    ...
    def _load_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'contacts': {'last_page': 0, 'completed': False, 'last_update': None},
            'companies': {'last_page': 0, 'completed': False, 'last_update': None},
            'events': {'last_page': 0, 'completed': False, 'last_update': None},
            'global': {
                'last_full_sync': None,
                'running_exports': []
            }
    ...
    def update_export_progress(self, entity_type: str, page: int, completed: bool = False):
        """Update the progress of an export"""
        if entity_type not in self.state:
            self.state[entity_type] = {'last_page': 0, 'completed': False, 'last_update': None}
    ...
    def mark_export_stopped(self, entity_type: str):
        ...
        if ('global' in self.state and 'running_exports' in self.state['global'] and
                entity_type in self.state['global']['running_exports']):
    ...
    def get_running_exports(self) -> list:
        ...
        if 'global' in self.state and 'running_exports' in self.state['global']:
