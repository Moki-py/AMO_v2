...
class Storage:
    ...
    def _initialize_files(self):
        ...
        for file_path in files:
            if not os.path.exists(file_path):
                ...
                log_event('storage', 'info', f'Initialized empty file: {file_path}')
    ...
    def _read_json_file(self, file_path: str) -> List[Dict[str, Any]]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                log_event('storage', 'error', f'Failed to decode JSON from {file_path} (attempt {attempt+1}/{max_retries})')
                ...
                log_event('storage', 'error', f'Error reading file {file_path}: {e}')
                # Wait before retrying
                time.sleep(retry_delay)

        # If all retries failed, return empty list
        log_event('storage', 'error', f'All attempts to read {file_path} failed, returning empty list')
        return []
    ...
    def _write_json_file(self, file_path: str, data: List[Dict[str, Any]]) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                log_event('storage', 'error', f'Error writing to {file_path} (attempt {attempt+1}/{max_retries}): {e}')
                # Wait before retrying
                time.sleep(retry_delay)

        # If all retries failed
        log_event('storage', 'error', f'All attempts to write to {file_path} failed')
        return False
    ...
    def save_entities(self, entity_type: str, entities: List[Dict[str, Any]]) -> bool:
        ...
        if not file_path:
            log_event('storage', 'error', f'Unknown entity type: {entity_type}')
            return False

        log_event('storage', 'info', f'Saving {len(entities)} {entity_type} to {file_path}')
    ...
    def append_entities(self, entity_type: str, entities: List[Dict[str, Any]]) -> bool:
        ...
        log_event('storage', 'info', f'Appending {len(entities)} {entity_type} to existing {len(existing_entities) - len(entities)}')
    ...
    def update_entity(self, entity_type: str, entity_id: int, entity_data: Dict[str, Any]) -> bool:
        ...
        entity_index = next((i for i, e in enumerate(entities)
    ...
    def delete_entity(self, entity_type: str, entity_id: int) -> bool:
        ...
        if len(entities) == len(updated_entities):
            log_event('storage', 'warning',
                     f'No {entity_type} with ID {entity_id} found to delete')
            return False

        log_event('storage', 'info', f'Deleted {entity_type} with ID {entity_id}')
    ...
    def add_log_entry(self, entry: Dict[str, Any]) -> bool:
        ...
        return self._write_json_file(config.LOG_FILE, logs)

    def _clean_old_logs(self, logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Remove log entries older than the retention period"""
        retention_date = datetime.now().timestamp() - (config.LOG_RETENTION_DAYS * 24 * 60 * 60)
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'logs': len(self._read_json_file(config.LOG_FILE))
        }